...
export class autoUpdate extends plugin {
  ...
  async runUpdate (plugin = '') {
    ...
    updataPluginLog.logs.push(`开始更新：${this.typeName}`);
    ...
    if (/Already up|已经是最新/g.test(ret.stdout)) {
      updataPluginLog.logs.push(`${this.typeName}已经是最新`);
    } else {
      this.isUp = true;
  ...
  async restart () {
    ...
    let packageManager = await this.checkPackageManager();
    try {
      await redis.set(this.key, 'autoUpdate', { EX: 120 });
      let cm = `${packageManager} start`;
      if (process.argv[1].includes('pm2')) {
        cm = `${packageManager} run restart`;
      } else {
        await this.reply('当前为前台运行，重启将转为后台...');
      }
      exec(cm, { windowsHide: true }, (error, stdout, stderr) => {
        if (error) {
          ...
          this.reply(`查看日志请用命令：${packageManager} run log`);
          this.reply(`停止后台运行命令：${packageManager} stop`);
          process.exit();
        }
      });
    } catch (error) {
  ...
  async checkPackageManager () {
    let yarn = await this.execSync('yarn -v');
    if (yarn.stdout) return 'yarn';

    let pnpm = await this.execSync('pnpm -v');
    if (pnpm.stdout) return 'pnpm';
